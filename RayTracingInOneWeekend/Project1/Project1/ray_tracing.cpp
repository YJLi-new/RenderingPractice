...
Vector3d ray_sphere_color(const ray& r, int bounce_num, hittableObjectsList project_scene) {
	if (bounce_num > 16) { return Vector3d::Zero(); }
	double t1 = INFINITYd;
	double t2 = INFINITYd;
	intersection inter;
	if (project_scene.intersect_ray(r, t1, t2, inter)) {
		...
		case DIFFUSE_NO_RANDOM:
			...
			return ray_sphere_color(ray(inter.position, new_ray_dir), bounce_num+1, project_scene).cwiseProduct(color);
			break;

		case METAL:
			new_ray_dir = reflect(r.get_direction(), inter.normal).normalized();
			return ray_sphere_color(ray(inter.position, new_ray_dir), 0, project_scene).cwiseProduct(color);
			break;
		...
		case DIFFUSE_NORMAL:
			...
			//Vector3d normalzd = (r.positionAtTheTime(t2) - sph.get_center());  // orig + h*dir/a - sqrtd*dir/a - center
			//normalzd.normalize();
		...
		case GLASS:
			...
			//color_at_ray = ray_sphere_color(ray(inter.position, new_ray_dir), bounce_num + 1, project_scene).cwiseProduct(color);
			//std::cout << "Color at Ray: (bounce) " << bounce_num << "(color) " << color_at_ray << "\n" << std::endl;
			//return color_at_ray;
			return ray_sphere_color(ray(inter.position, new_ray_dir), bounce_num + 1, project_scene).cwiseProduct(color);
			break;
